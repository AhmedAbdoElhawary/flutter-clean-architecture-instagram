...
class CommentLikesCubit extends Cubit<CommentLikesState> {
  ...
  CommentLikesCubit(this._putLikeOnThisCommentUseCase, this._removeLikeOnThisCommentUseCase)
      : super(CommentLikesInitial());

  static CommentLikesCubit get(BuildContext context) => BlocProvider.of(context);

  Future<void> putLikeOnThisComment(
      {required String postId, required String commentId, required String myPersonalId}) async {
    emit(CubitCommentLikesLoading());

    await _putLikeOnThisCommentUseCase.call(paramsOne: commentId, paramsTwo: myPersonalId).then((_) {
  ...
  Future<void> removeLikeOnThisComment(
    ...
    await _removeLikeOnThisCommentUseCase.call(paramsOne: commentId, paramsTwo: myPersonalId).then((_) {
      emit(CubitCommentLikesLoaded());
    }).catchError((e) {
      emit(CubitCommentLikesFailed(e.toString()));
    });
