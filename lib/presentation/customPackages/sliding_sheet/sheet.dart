...
import 'package:instagram/presentation/customPackages/sliding_sheet/sheet_dialog.dart';
...
class SlidingSheetState extends State<SlidingSheet>
  ...
  SheetState get state => SheetState(
    extent,
    extent: reverseSnap(currentExtent),
    minExtent: reverseSnap(minExtent),
    maxExtent: reverseSnap(maxExtent),
    isLaidOut: isLaidOut,
  );
  ...
  double reverseSnap(double snap) {
...
class SheetController {
  ...
  static SheetController? of(BuildContext context) {
    return context
        .findAncestorStateOfType<SlidingSheetState>()
        ?.sheetController;
  }
