...
class ImageCrop {
  ...
  static Future<ImageOptions> getImageOptions({
    ...
    final result =
        await _channel.invokeMethod('getImageOptions', {'path': file});
  ...
  static Future<Uint8List> cropImage({
    ...
    double? scale,
      ...
      _channel.invokeMethod('cropImage', {
        ...
        'bottom': area.bottom,
        'scale': scale ?? 1.0,
      }).then<Uint8List>((result) =>result);

  static Future<Uint8List> sampleImage({
    required Uint8List file,
    int? preferredSize,
    int? preferredWidth,
    int? preferredHeight,
    ...
    final Uint8List path = await _channel.invokeMethod('sampleImage', {
      'path': file,
