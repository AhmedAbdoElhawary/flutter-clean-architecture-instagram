...
class FirebaseUserRepoImpl implements FirestoreUserRepository {
  ...
  Future<void> addNewUser(UserPersonalInfo newUserInfo) async {
    try {
      ...
      await FireStoreNotification.createNewDeviceToken(userId: newUserInfo.userId, myPersonalInfo: newUserInfo);
    } catch (e) {
  ...
  @override
  Future<UserPersonalInfo> getPersonalInfo({required String userId, bool getDeviceToken = false}) async {
    try {
      ...
      if (isThatMobile && getDeviceToken) {
        UserPersonalInfo updateInfo =
            await FireStoreNotification.createNewDeviceToken(userId: userId, myPersonalInfo: myPersonalInfo);
  ...
  @override
  Future<UserPersonalInfo> updateUserInfo({required UserPersonalInfo userInfo}) async {
    try {
  ...
  @override
  Future<UserPersonalInfo> updateUserPostsInfo({required String userId, required Post postInfo}) async {
    try {
  ...
  Future<String> uploadProfileImage(
    ...
    try {
      String imageUrl = await FirebaseStoragePost.uploadData(data: photo, folderName: 'personalImage');
  ...
  Future<FollowersAndFollowingsInfo> getFollowersAndFollowingsInfo(
    ...
    try {
      List<UserPersonalInfo> followersInfo = await FireStoreUser.getSpecificUsersInfo(
          usersIds: followersIds, fieldName: "followers", userUid: myPersonalId);
      List<UserPersonalInfo> followingsInfo = await FireStoreUser.getSpecificUsersInfo(
          usersIds: followingsIds, fieldName: "following", userUid: myPersonalId);
      return FollowersAndFollowingsInfo(followersInfo: followersInfo, followingsInfo: followingsInfo);
    } catch (e) {
      return Future.error(e.toString());
    }
  }
  ...
  @override
  Future<void> followThisUser(String followingUserId, String myPersonalId) async {
    try {
  ...
  @override
  Future<void> unFollowThisUser(String followingUserId, String myPersonalId) async {
    try {
      return await FireStoreUser.unFollowThisUser(followingUserId, myPersonalId);
    } catch (e) {
      return Future.error(e.toString());
    }
  }
  ...
  Future<Message> sendMessage({required Message messageInfo, Uint8List? pathOfPhoto, required File? recordFile}) async {
    try {
      ...
      Message myMessageInfo = await FireStoreSingleChat.sendMessage(
          userId: messageInfo.senderId, chatId: messageInfo.receiversIds[0], message: messageInfo);

      await FireStoreSingleChat.sendMessage(
          userId: messageInfo.receiversIds[0], chatId: messageInfo.senderId, message: messageInfo);

      await FireStoreUser.sendNotification(userId: messageInfo.receiversIds[0], message: messageInfo);
  ...
  Future<void> deleteMessage(
      {required Message messageInfo, Message? replacedMessage, required bool isThatOnlyMessageInChat}) async {
    try {
      ...
      for (int i = 0; i < 2; i++) {
        ...
        await FireStoreSingleChat.deleteMessage(userId: userId, chatId: chatId, messageId: messageInfo.messageUid);
  ...
  @override
  Future<SenderInfo> getSpecificChatInfo({required String chatUid, required bool isThatGroup}) async {
    try {
      if (isThatGroup) {
        SenderInfo coverChatInfo = await FireStoreGroupChat.getChatInfo(chatId: chatUid);
        SenderInfo messageDetails = await FireStoreUser.extractUsersForGroupChatInfo(coverChatInfo);
        return messageDetails;
      } else {
        SenderInfo coverChatInfo = await FireStoreUser.getChatOfUser(chatUid: chatUid);
        SenderInfo messageDetails = await FireStoreUser.extractUsersForSingleChatInfo(coverChatInfo);
        return messageDetails;
      }
    } catch (e) {
      return Future.error(e.toString());
  ...
  @override
  Future<List<SenderInfo>> getChatUserInfo({required UserPersonalInfo myPersonalInfo}) async {
    try {
      List<SenderInfo> allChatsOfGroupsInfo =
          await FireStoreGroupChat.getSpecificChatsInfo(chatsIds: myPersonalInfo.chatsOfGroups);
      List<SenderInfo> allChatsInfo = await FireStoreUser.getMessagesOfChat(userId: myPersonalInfo.userId);
      List<SenderInfo> allChats = allChatsInfo + allChatsOfGroupsInfo;
      List<SenderInfo> allUsersInfo = await FireStoreUser.extractUsersChatInfo(messagesDetails: allChats);
      return allUsersInfo;
    } catch (e) {
      return Future.error(e.toString());
    }
