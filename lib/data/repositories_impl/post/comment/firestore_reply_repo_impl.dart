...
class FireStoreRepliesRepositoryImpl implements FirestoreReplyRepository {
  ...
  Future<Comment> replyOnThisComment({required Comment replyInfo}) async {
    try {
      String replyId = await FireStoreReply.replyOnThisComment(replyInfo: replyInfo);

      Comment theReplyInfo = await FireStoreReply.getReplyInfo(replyId: replyId);
      await FirestoreComment.putReplyOnThisComment(
          commentId: theReplyInfo.parentCommentId, replyId: theReplyInfo.commentUid);
  ...
  Future<void> putLikeOnThisReply({required String replyId, required String myPersonalId}) async {
    try {
      await FireStoreReply.putLikeOnThisReply(replyId: replyId, myPersonalId: myPersonalId);
    } catch (e) {
      return Future.error(e.toString());
    }
  }
  ...
  Future<void> removeLikeOnThisReply({required String replyId, required String myPersonalId}) async {
    try {
      await FireStoreReply.removeLikeOnThisReply(replyId: replyId, myPersonalId: myPersonalId);
    } catch (e) {
      return Future.error(e.toString());
    }
  }
  ...
  Future<List<Comment>> getSpecificReplies({required String commentId}) async {
    try {
      List? repliesIds = await FirestoreComment.getRepliesOfComments(commentId: commentId);
      List<Comment> theRepliesInfo = await FireStoreReply.getSpecificReplies(repliesIds: repliesIds!);
