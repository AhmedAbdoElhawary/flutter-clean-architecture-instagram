...
class FireStoreSingleChat {
  ...
  static Future<Message> sendMessage({
    ...
    DocumentReference<Map<String, dynamic>> fireChatsCollection = userCollection.collection("chats").doc(chatId);
  ...
  static Future<void> updateLastMessage({
    ...
    if (message != null) {
      Map<String, dynamic> toMap = isThatOnlyMessageInChat ? {"": ""} : message.toMap();
      await fireChatsCollection.set(toMap);
    }
  }

  static Stream<List<Message>> getMessages({required String receiverId}) {
    Stream<QuerySnapshot<Map<String, dynamic>>> snapshotsMessages = _fireStoreUserCollection
        .doc(myPersonalId)
        .collection("chats")
        .doc(receiverId)
        .collection("messages")
  ...
  static Future<void> deleteMessage({required String userId, required String chatId, required String messageId}) async {
